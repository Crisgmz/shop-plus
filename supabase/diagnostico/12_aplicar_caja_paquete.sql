-- ============================================================================
-- APLICAR: dejar TODOS los productos de la hoja en formato CAJA → PAQUETE
...
-- presentación).
--
-- ── ESTE ARCHIVO SÍ CAMBIA DATOS ───────────────────────────────────────────
--   Es el 11 con `aplicar` fijo en `true`, para no depender de editar una
--   línea. Ya se revisó la vista previa del 11 (16/09/2026).
--
--   1. En el SQL Editor abra una pestaña NUEVA y pegue TODO el archivo.
--   2. NO seleccione nada: con texto seleccionado, Run corre solo eso.
--   3. Run. Si avisa de "operación destructiva", confirme (es la tabla
--      temporal `_plan` de este mismo script).
--   4. La tabla final debe decir APLICADO en `modo`, 21 "✔ convertido" y
--      21 "✔ gemelo desactivado". Si sale un error, nada se guardó: copie
--      el mensaje completo.
--   Correrlo de nuevo no hace daño: lo ya convertido no se toca.
-- ============================================================================
...
config(aplicar) as (values
  (true)    -- este archivo aplica; la vista previa es el 11
),
