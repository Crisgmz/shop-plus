-- ============================================================================
-- 20260916_92_precio_por_presentacion.sql
--
-- La CAJA se cobra a su precio, no al unitario × unidades.
--
-- Caso real: una caja de 20 paquetes vale RD$2,639.83 → RD$131.9915 el paquete.
-- Ese precio no existe en centavos: 131.99 × 20 = 2,639.80, tres centavos menos
-- en cada caja. Y al revés, el precio de detalle es más del doble que el de
-- mayoreo, así que derivar el precio de la caja del unitario no sirve.
--
-- Cada línea puede venir ahora con su presentación:
--
--   uom         'pack' | 'box' | 'unit'
--   uom_factor  unidades base que representa UNA (1 caja = 20 paquetes)
--   uom_price   precio de UNA presentación (2,639.83)
--   unit_name   cómo se llama en la factura ("Caja")
--
-- Con `uom_price`, el bruto de la línea es `uom_price × (quantity / uom_factor)`
-- — exacto al centavo. Sin esos campos, se calcula como siempre: `unit_price ×
-- quantity`. El otro app (flutter_shop+) no los manda y no cambia en nada.
--
-- NO cambia la firma de ninguna función: solo el cuerpo. Los cuatro campos
-- viajan dentro de `p_items`, que ya es jsonb libre.
--
-- Además el checkout ahora GUARDA la presentación en `sale_items` al crear la
-- venta. Antes la marcaba después `tag_sale_item_presentations` (migración 89),
-- que sigue existiendo como respaldo pero ya no hace falta: así es atómico y la
-- factura puede imprimir el precio exacto de la caja.
--
-- Ejecutar en el SQL Editor de Supabase, DESPUÉS de la 91.
...
comment on column public.sale_items.uom_price is
  'Precio de UNA presentación vendida (1 caja = 2,639.83). NULL = línea por unidad base.';
...
begin
  ...
  if p_client_id is not null then
    select c.id, c.full_name, c.is_active, c.tax_exempt, c.charge_itbis
    into v_client
    ...
    -- CAMBIO (migración 91): si el app lo pide (`p_honor_client_tax`), un
    -- cliente con "Cobrar ITBIS" apagado o marcado exento no factura ITBIS.
    v_client_no_tax := coalesce(p_honor_client_tax, false)
      and (coalesce(v_client.tax_exempt, false)
           or not coalesce(v_client.charge_itbis, true));
  end if;
  ...
  create temp table if not exists tmp_hold_items (
    ...
    imeis text[],
    -- Presentación vendida (migración 92): "1 Caja" en vez de "20 paquetes".
    uom text,
  ...
  for v_item in
    select
      ...
      coalesce((item->>'discount_amount')::numeric, 0)::numeric(14,2)
        as discount_amount,
      -- Presentación (migración 92). Si no vienen, la línea es por unidad
      -- base y todo se calcula como antes: el otro app no las manda.
  ...
  loop
    ...
    -- CAMBIO (migración 92): si la línea trae el precio de la PRESENTACIÓN
    -- (1 caja a 2,639.83), el bruto sale de ahí. Derivarlo del precio por
    -- unidad base obliga a redondear al centavo por unidad y la caja nunca
    -- daría su precio exacto. Sin esos campos, se calcula igual que siempre.
    if v_item.uom_price is not null and coalesce(v_item.uom_factor, 0) > 0 then
      v_gross := round(
        (v_item.uom_price * (v_item.quantity / v_item.uom_factor))::numeric, 2
      );
    else
    ...
    insert into tmp_hold_items (
      ...
      coalesce(v_item.uom, 'unit'),
      coalesce(v_item.uom_factor, 1),
      v_item.uom_price,
      v_item.unit_name
    );
...
grant execute on function public.hold_sale_transactional(
  jsonb, text, uuid, text, uuid, boolean
) to authenticated;
...
grant execute on function public.checkout_sale_transactional(
  jsonb, text, boolean, text, uuid, text, integer, uuid, jsonb, uuid, boolean
) to authenticated;
...
begin
  ...
  -- CAMBIO (migración 91): el cliente con que QUEDA la venta decide el ITBIS,
  -- igual que en el checkout, si el app lo pide.
  ...
  loop
    ...
    declare
      ...
      -- CAMBIO (migración 92): el bruto sale del precio de la presentación
      -- cuando la línea lo trae; si no, de unitario × unidades (ver checkout).
