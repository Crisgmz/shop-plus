...
--
-- Completa la venta SIN comprobante (nota de venta no fiscal).
--
-- La migración 59 introdujo `receipt_type = 'none'` y enseñó a los triggers de
-- NCF y de validación fiscal a saltearlo. Pero quedaron dos huecos que hacían
-- que la opción "Sin comprobante" del POS fallara al cobrar:
--
--   1) `normalize_receipt_type('none')` seguía lanzando
--      "Tipo de comprobante no soportado: none" — y todos los RPC de venta
--      pasan el tipo por ahí, así que el checkout abortaba.
--   2) Aun pasando la validación, la venta cobraba ITBIS: el RPC calcula el
--      impuesto leyendo `products.tax_rate` (ignora el `tax_rate` que manda el
--      cliente), y no tenía forma de saber que la venta no es fiscal.
--
-- Esta migración:
--   1) `normalize_receipt_type` acepta 'none' / 'sin comprobante'.
--   2) `checkout_sale_transactional`, `hold_sale_transactional` y
--      `edit_sale_transactional` calculan la tasa efectiva por línea: 0 si la
--      venta es 'none' o el producto es exento; la del producto en cualquier
--      otro caso. Así subtotal = total, y `paid_amount`, `balance_due` y las
--      filas de `payments` cuadran — por eso el cambio va en los RPC (donde se
--      suman los totales) y no en un trigger.
--
-- Los tres RPC son copia FIEL de su última versión (migración 54 el checkout y
-- el hold, migración 47 la edición); el ÚNICO cambio es la tasa de impuesto por
-- línea (`v_line_tax_rate` / `v_rate`).
--
-- Ejecutar en el SQL Editor de Supabase, DESPUÉS de la migración 59.
-- Idempotente (CREATE OR REPLACE).
-- ============================================================================

-- ----------------------------------------------------------------------------
-- 1) normalize_receipt_type: aceptar 'none' (venta sin comprobante fiscal).
-- ----------------------------------------------------------------------------
...
begin
  ...
  case v_normalized
    -- Sin comprobante: nota de venta no fiscal. No consume NCF ni lleva ITBIS.
    when 'none', 'sin_comprobante', 'ninguno' then
      return 'none'::public.receipt_type;
...
-- ----------------------------------------------------------------------------
-- 2) hold_sale_transactional — copia de la 54 + v_line_tax_rate.
-- ----------------------------------------------------------------------------
  ...
  loop
    ...
    values (
      ...
      round((v_item.unit_price * v_item.quantity
             * v_line_tax_rate / 100)::numeric, 2),
      round((v_item.unit_price * v_item.quantity
             * (1 + v_line_tax_rate / 100))::numeric, 2),
...
-- ----------------------------------------------------------------------------
-- 3) checkout_sale_transactional — copia de la 54 + v_line_tax_rate.
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 4) edit_sale_transactional — copia de la 47 + regla 'none'.
-- ----------------------------------------------------------------------------
-- Editar una venta recalcula las líneas desde `products.tax_rate`. Sin esta
-- regla, editar una venta sin comprobante le agregaba ITBIS y el total dejaba
-- de coincidir con lo cobrado. Se agrega `receipt_type` al SELECT de la venta
-- para poder aplicarla; el resto es copia fiel de la migración 47.
...
begin
  ...
  loop
    ...
    declare
      -- Una venta SIN comprobante es una nota de venta no fiscal: no lleva
      -- ITBIS. Mismo criterio que checkout_sale_transactional; si no, editar
      -- una venta 'none' le agregaría impuesto y descuadraría la caja.
...
-- Recargar el cache de esquema de PostgREST para que los RPC queden llamables
-- de inmediato (mismo paso que usan las migraciones anteriores del checkout).
