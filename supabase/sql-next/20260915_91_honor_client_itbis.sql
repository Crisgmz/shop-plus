-- ============================================================================
-- 20260915_91_honor_client_itbis.sql
--
-- Respetar "Cobrar ITBIS" y "Exento de impuestos" del CLIENTE al cobrar.
--
-- `clients.charge_itbis` y `clients.tax_exempt` existen desde abril y se editan
-- en la ficha del cliente, pero ninguna función de cobro los leía: una venta
-- con consumidor final a un cliente con "Cobrar ITBIS" apagado seguía
-- facturando ITBIS. Tampoco el POS los consideraba.
--
-- ⚠️ Estas funciones las comparten DOS negocios (shop-plus y flutter_shop+).
-- Para no cambiar cómo factura el otro, la regla es OPT-IN: un parámetro nuevo
-- `p_honor_client_tax boolean default false`. Quien no lo manda —hoy
-- flutter_shop+— sigue exactamente igual. shop-plus lo manda en `true`.
--
-- Regla: con `p_honor_client_tax = true`, si el cliente tiene
-- `tax_exempt = true` o `charge_itbis = false`, la tasa de cada línea es 0.
-- Va en la tasa por línea —donde ya viven 'none' y el producto exento— para
-- que subtotal, total, `paid_amount` y `payments` sigan cuadrando.
--
-- Cambian de firma (un parámetro más al final), así que se eliminan las
-- versiones anteriores en la misma transacción: si quedaran las dos, una
-- llamada sin el parámetro sería ambigua ("function is not unique").
--
-- Las tres funciones son copia FIEL de la migración 83; los ÚNICOS cambios son
-- el parámetro, la lectura de las dos banderas y la regla en la tasa.
--
-- Ejecutar en el SQL Editor de Supabase, DESPUÉS de la migración 83.
-- ============================================================================

begin;

drop function if exists public.hold_sale_transactional(
